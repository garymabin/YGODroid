...
bool DuelClient::StartClient(unsigned int ip, unsigned short port, CLIENT_EVT_PARAM_COMPAT* param) {
	if(connect_state)
		return false;
	sockaddr_in sin;
	...
	bufferevent_setcb(client_bev, ClientRead, NULL, ClientEvent, (void*)param);
...
//		mainGame->gMutex.Unlock();
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_CONNECTION_TIMEOUT);
...
void DuelClient::ClientEvent(bufferevent *bev, short events, void *ctx) {
	if (events & BEV_EVENT_CONNECTED) {
		CLIENT_EVT_PARAM_COMPAT* param = reinterpret_cast<CLIENT_EVT_PARAM_COMPAT*>(ctx);
		bool create_game = param->event_type != 0;
//		CTOS_PlayerInfo cspi;
//		BufferIO::CopyWStr(mainGame->ebNickName->getText(), cspi.name, 20);
		SendPacketToServer(CTOS_PLAYER_INFO, *(param->ctos_playerinfo));
		if(create_game) {
...
//			cscg.info.no_shuffle_deck = mainGame->chkNoShuffleDeck->isChecked();
			SendPacketToServer(CTOS_CREATE_GAME, *(param->ctos_creategame));
		} else {
//			CTOS_JoinGame csjg;
//			csjg.version = PRO_VERSION;
//			csjg.gameid = 0;
//			BufferIO::CopyWStr(mainGame->ebJoinPass->getText(), csjg.pass, 20);
			SendPacketToServer(CTOS_JOIN_GAME, *(param->ctos_joingame));
		}
...
//				mainGame->gMutex.Unlock();
				...
				YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_CONNECTION_TIMEOUT, (CCObject*)(events & BEV_EVENT_EOF));
...
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	case STOC_ERROR_MSG: {
		...
		case ERRMSG_JOINERROR: {
			YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_JOIN_ERROR, (CCObject*)pkt->code);
//			mainGame->btnCreateHost->setEnabled(true);
//			mainGame->btnJoinHost->setEnabled(true);
//			mainGame->btnJoinCancel->setEnabled(true);
//			mainGame->gMutex.Lock();
...
//			mainGame->gMutex.Unlock();
		...
		case ERRMSG_DECKERROR: {
			YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_DECK_ERROR, (CCObject*)pkt->code);
//			mainGame->gMutex.Lock();
//			if(pkt->code == 1)
...
//			mainGame->gMutex.Unlock();
		...
		case ERRMSG_SIDEERROR: {
			YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_SIDE_ERROR);
//			mainGame->gMutex.Lock();
...
//			mainGame->gMutex.Unlock();
		...
		case ERRMSG_VERERROR: {
			YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_VERSION_ERROR, (CCObject*)pkt->code);
//			mainGame->btnCreateHost->setEnabled(true);
//			mainGame->btnJoinHost->setEnabled(true);
//			mainGame->btnJoinCancel->setEnabled(true);
//			mainGame->gMutex.Lock();
...
//			event_base_loopbreak(client_base);
	...
	case STOC_SELECT_HAND: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_SELECT_HAND);
//		mainGame->wHand->setVisible(true);
		break;
	}
	case STOC_SELECT_TP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_SELECT_TP);
//		mainGame->gMutex.Lock();
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_HAND_RESULT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEventBlocked(YGO_EVENT_TYPE_HAND_RESULT, 60, (CCObject*)pdata);
...
//		mainGame->WaitFrameSignal(60);
	...
	case STOC_CHANGE_SIDE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_CHANGE_SIDE);
//		mainGame->gMutex.Lock();
//		mainGame->dInfo.isStarted = false;
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_WAITING_SIDE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_WAITING_SIDE);
//		mainGame->gMutex.Lock();
//		mainGame->dField.Clear();
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_JOIN_GAME: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_JOIN_GAME, (CCObject*)pdata);
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_TYPE_CHANGE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_TYPE_CHANGE, (CCObject*)pdata);
...
//		mainGame->dInfo.player_type = selftype;
	...
	case STOC_DUEL_START: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEventBlocked(YGO_EVENT_TYPE_DUEL_START, 11);
//		mainGame->HideElement(mainGame->wHostPrepare);
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_DUEL_END: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_DUEL_END);
//		mainGame->gMutex.Lock();
...
//			mainGame->device->closeDevice();
	...
	case STOC_REPLAY: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_REPLAY);
//		mainGame->gMutex.Lock();
...
//		}
	...
	case STOC_TIME_LIMIT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_TIME_LIMIT, (CCObject*)pdata);
...
//		mainGame->dInfo.time_left[lplayer] = pkt->left_time;
	...
	case STOC_CHAT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_CHAT, (CCObject*)pdata);
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_HS_PLAYER_ENTER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_HOST_PLAYER_ENTER, (CCObject*)pdata);
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_HS_PLAYER_CHANGE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_HOST_PLAYER_CHANGE, (CCObject*)pdata);
...
//		mainGame->gMutex.Unlock();
	...
	case STOC_HS_WATCH_CHANGE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_HOST_WATCH_CHANGE, (CCObject*)pdata);
...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	DuelInfo duelInfo = YGOGameConfig::sharedGameConfig()->m_duelInfo;
//	mainGame->dInfo.curMsg = BufferIO::ReadUInt8(pbuf);
	duelInfo.curMsg = BufferIO::ReadUInt8(pbuf);
	mainGame->wCmdMenu->setVisible(false);
	if(!duelInfo.isReplay && duelInfo.curMsg != MSG_WAITING && duelInfo.curMsg != MSG_CARD_SELECTED) {
		//FIXME:需要定义一类消息来处理此情况，具体暂不明
...
//		}
	...
	if(duelInfo.time_player == 1)
		duelInfo.time_player = 2;
	switch(duelInfo.curMsg) {
	case MSG_RETRY: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_RETRY);
//		mainGame->gMutex.Lock();
...
//			mainGame->device->closeDevice();
	...
	case MSG_HINT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_HINT, (CCObject*)pbuf);
...
//		}
	...
	case MSG_WIN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_WIN, (CCObject*)pbuf);
//		int player = BufferIO::ReadInt8(pbuf);
//		int type = BufferIO::ReadInt8(pbuf);
...
//		mainGame->showcard = 0;
	...
	case MSG_WAITING: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_WAITING);
...
//		mainGame->showcardcode = 11;
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_START);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_UPDATE_DATA: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_UPDATE_DATA);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_UPDATE_CARD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_UPDATE_CARD);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_BATTLECMD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_BATTLE_CMD);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_IDLECMD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_IDLE_CMD);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_SELECT_EFFECTYN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_EFFECTYN);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_YESNO: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_YESNO);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_OPTION: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_OPTION);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_CARD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_CARD);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_SELECT_CHAIN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_CHAIN);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
//		int count = BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_DISFIELD: {
		//FIXME:需要记住这里的处理似乎不涉及UI，而且需要同步返回
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_PLACE);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
//		mainGame->dField.select_min = BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_SELECT_POSITION: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_POSITION);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_TRIBUTE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_TRIBUTE);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		mainGame->stHintMsg->setVisible(false);
	...
	case MSG_SELECT_COUNTER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_COUNTER);
//		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_SUM: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SELECT_SUM);
...
//		}
	...
	case MSG_SORT_CARD:
	case MSG_SORT_CHAIN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SORT_CARD);
//		/*int player = */BufferIO::ReadInt8(pbuf);
//		int count = BufferIO::ReadInt8(pbuf);
//		mainGame->dField.selectable_cards.clear();
//		mainGame->dField.selected_cards.clear();
...
//		mainGame->dField.ShowSelectCard();
	...
	case MSG_CONFIRM_DECKTOP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CONFIRM_DESKTOP);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
//		int code;
//		ClientCard* pcard;
...
//		}
	...
	case MSG_CONFIRM_CARDS: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CONFIRM_CARDS);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
//		int code, c, l, s;
...
//		}
	...
	case MSG_SHUFFLE_DECK: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SHUFFLE_HAND);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		}
	...
	case MSG_REFRESH_DECK: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_REFRESH_DECK);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
		return true;
	}
	case MSG_SWAP_GRAVE_DECK: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SWAP_GRAVE_DECK);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
...
//		}
	...
	case MSG_DECK_TOP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_DECK_TOP);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int seq = BufferIO::ReadInt8(pbuf);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
...
//		}
	...
	case MSG_SHUFFLE_SET_CARD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SHUFFLE_SET_CARD);
...
//		}
	...
	case MSG_NEW_TURN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_NEW_TURN);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		mainGame->dInfo.turn++;
...
//		}
	...
	case MSG_NEW_PHASE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_NEW_PHASE);
...
//		}
	...
	case MSG_MOVE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_MOVE);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
...
//		}
	...
	case MSG_POS_CHANGE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_POS_CHANGE);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
//		int cc = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		}
	...
	case MSG_SET: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SET);
...
//		myswprintf(event_string, dataManager.GetSysString(1601));
	...
	case MSG_SWAP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SWAP);
...
//		}
	...
	case MSG_FIELD_DISABLED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_FIELD_DISABLED);
...
//		mainGame->dField.disabled_field = disabled;
	...
	case MSG_SUMMONING: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SUMMONING);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
//		/*int cc = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		/*int cl = */BufferIO::ReadInt8(pbuf);
//		/*int cs = */BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_SUMMONED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SUMMONED);
//		myswprintf(event_string, dataManager.GetSysString(1604));
		return true;
	}
	case MSG_SPSUMMONING: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SPSUMMONING);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
//		/*int cc = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		/*int cl = */BufferIO::ReadInt8(pbuf);
//		/*int cs = */BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_SPSUMMONED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_SPSUMMONED);
...
//		}
	...
	case MSG_FLIPSUMMONED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_FLIPSUMMONED);
//		myswprintf(event_string, dataManager.GetSysString(1608));
		return true;
	}
	case MSG_CHAINING: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAINING);
//		unsigned int code = (unsigned int)BufferIO::ReadInt32(pbuf);
...
//		return true;
	...
	case MSG_CHAINED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAINED);
//		int ct = BufferIO::ReadInt8(pbuf);
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping)
//			return true;
...
//		mainGame->dField.last_chain = true;
	...
	case MSG_CHAIN_SOLVING: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAIN_SOLVING);
//		int ct = BufferIO::ReadInt8(pbuf);
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping)
//			return true;
...
//		mainGame->dField.last_chain = false;
	...
	case MSG_CHAIN_SOLVED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAIN_SOLVED);
//		/*int ct = */BufferIO::ReadInt8(pbuf);
		return true;
	}
	case MSG_CHAIN_END: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAIN_END);
//		mainGame->dField.chains.clear();
		return true;
	}
	case MSG_CHAIN_NEGATED:
	case MSG_CHAIN_DISABLED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CHAIN_DISABLED);
//		int ct = BufferIO::ReadInt8(pbuf);
//		if(!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
...
//		}
	...
	case MSG_RANDOM_SELECTED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_RANDOM_SELECTED);
//		/*int player = */BufferIO::ReadInt8(pbuf);
//		int count = BufferIO::ReadInt8(pbuf);
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
//			pbuf += count * 4;
...
//			pcards[i]->is_highlighting = false;
	...
	case MSG_BECOME_TARGET: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_BECOME_TARGET);
//		int count = BufferIO::ReadInt8(pbuf);
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
//			pbuf += count * 4;
//			return true;
...
//		}
	...
	case MSG_DRAW: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_DRAW);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
//		ClientCard* pcard;
//		for (int i = 0; i < count; ++i) {
...
//		else myswprintf(event_string, dataManager.GetSysString(1612), count);
	...
	case MSG_DAMAGE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_DAMAGE);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_RECOVER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_RECOVER);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int val = BufferIO::ReadInt32(pbuf);
//		int final = mainGame->dInfo.lp[player] + val;
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_EQUIP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_EQUIP);
//		int c1 = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l1 = BufferIO::ReadInt8(pbuf);
//		int s1 = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_LPUPDATE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_LPUPDATE);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int val = BufferIO::ReadInt32(pbuf);
//		if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_UNEQUIP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_UNEQUIP);
//		int c1 = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l1 = BufferIO::ReadInt8(pbuf);
//		int s1 = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_CARD_TARGET: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_TARGET);
//		int c1 = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l1 = BufferIO::ReadInt8(pbuf);
//		int s1 = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_CANCEL_TARGET: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CANCEL_TARGET);
//		int c1 = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l1 = BufferIO::ReadInt8(pbuf);
//		int s1 = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_PAY_LPCOST: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_PAY_LPCOST);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_ADD_COUNTER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ADD_COUNTER);
...
//		pc->is_highlighting = false;
	...
	case MSG_REMOVE_COUNTER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_REMOVE_COUNTER);
//		int type = BufferIO::ReadInt16(pbuf);
//		int c = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l = BufferIO::ReadInt8(pbuf);
//		int s = BufferIO::ReadInt8(pbuf);
...
//		pc->is_highlighting = false;
	...
	case MSG_ATTACK: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ATTACK);
...
//		mainGame->is_attacking = false;
	...
	case MSG_BATTLE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_BATTLE);
//		int ca = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int la = BufferIO::ReadInt8(pbuf);
//		int sa = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_ATTACK_DISABLED: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ATTACK_DISABLED);
//		myswprintf(event_string, dataManager.GetSysString(1621), dataManager.GetName(mainGame->dField.attacker->code));
		return true;
	}
	case MSG_DAMAGE_STEP_START: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_DAMAGE_STEP_START);
		return true;
	}
	case MSG_DAMAGE_STEP_END: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_DAMAGE_STEP_END);
		return true;
	}
	case MSG_MISSED_EFFECT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_MISSED_EFFECT);
...
//		mainGame->logParam.push_back(code);
	...
	case MSG_TOSS_COIN: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_TOSS_COIN);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
...
//		mainGame->WaitFrameSignal(40);
	...
	case MSG_TOSS_DICE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_TOSS_DICE);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
//		wchar_t* pwbuf = textBuffer;
...
//		mainGame->WaitFrameSignal(40);
	...
	case MSG_ANNOUNCE_RACE: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ANNOUNCE_RACE);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_ANNOUNCE_ATTRIB: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ANNOUNCE_ATTRIB);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		mainGame->dField.announce_count = BufferIO::ReadInt8(pbuf);
//		int available = BufferIO::ReadInt32(pbuf);
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_ANNOUNCE_CARD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ANNOUNCE_CARD);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		if(select_hint)
//			myswprintf(textBuffer, L"%ls", dataManager.GetDesc(select_hint));
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_ANNOUNCE_NUMBER: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_ANNOUNCE_NUMBER);
//		/*int player = */mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int count = BufferIO::ReadInt8(pbuf);
//		mainGame->gMutex.Lock();
...
//		mainGame->gMutex.Unlock();
	...
	case MSG_CARD_HINT: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_CARD_HINT);
//		int c = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
//		int l = BufferIO::ReadInt8(pbuf);
//		int s = BufferIO::ReadInt8(pbuf);
//		BufferIO::ReadInt8(pbuf);
...
//		}
	...
	case MSG_MATCH_KILL: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_MATCH_KILL);
//		match_kill = BufferIO::ReadInt32(pbuf);
		return true;
	}
	case MSG_TAG_SWAP: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_TAG_SWAP);
//		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
//		}
	...
	case MSG_RELOAD_FIELD: {
		YGONetworkEventAdapter::sharedEventAdapter()->postEvent(YGO_EVENT_TYPE_MSG_RELOAD_FIELD);
//		mainGame->gMutex.Lock();
//		mainGame->dField.Clear();
//		int val = 0;
//		for(int p = 0; p < 2; ++p) {
